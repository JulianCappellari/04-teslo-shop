...
interface State {
  ...
  actualizarCantidadDeProductos: (producto: ProductoDelCarrito, cantidad:number) => void
  ...
  persist(
    (set, get) => ({
      ...
      getSumaInformacion: () => {
        ...
        const subTotal = carrito.reduce( (subTotal, producto) => producto.cantidad * producto.precio + subTotal, 0)
        const impuestos = subTotal * 0.21
        const total = subTotal + impuestos
        const itemsEnElCarrito = carrito.reduce( (total, item) => total + item.cantidad, 0)
      ...
      actualizarCantidadDeProductos: (producto: ProductoDelCarrito, cantidad:number) => {
        const {carrito} = get()
      ...
      eliminarProducto:(producto: ProductoDelCarrito) => {
        const {carrito} = get()
        const productoAEliminar = carrito.filter( item => item.id !== producto.id ||  item.talle !== producto.talle)
